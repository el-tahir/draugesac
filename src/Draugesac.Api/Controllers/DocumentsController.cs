...
public class DocumentsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllDocuments()
    {
        try
        {
            ...
            var response = documents.Select(document => new
            {
                Id = document.Id,
    ...
    public async Task<IActionResult> UploadDocument(IFormFile file)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Upload attempt with file too large: {FileSize} bytes", file.Length);
                return BadRequest(new { Error = "File size exceeds 10MB limit" });
            }

            // Validate file type (basic validation)
            string[] allowedExtensions = new[] { ".pdf", ".doc", ".docx", ".txt" };
            string? fileExtension = Path.GetExtension(file.FileName)?.ToLowerInvariant();
            if (string.IsNullOrEmpty(fileExtension) || !allowedExtensions.Contains(fileExtension))
            {
                _logger.LogWarning("Upload attempt with unsupported file type: {FileExtension}", fileExtension);
                return BadRequest(new { Error = "Unsupported file type. Allowed types: PDF, DOC, DOCX, TXT" });
            }
    ...
    public async Task<IActionResult> StartRedaction(Guid id, [FromBody] RedactionRequest request)
    {
        try
        {
            ...
            if (request?.PhrasesToRedact == null || !request.PhrasesToRedact.Any())
            {
                _logger.LogWarning("Redaction request for document {DocumentId} has no phrases to redact", id);
                return BadRequest(new { Error = "No phrases to redact provided" });
            }

            // Validate phrases are not empty or whitespace
            var validPhrases = request.PhrasesToRedact
            ...
            if (!validPhrases.Any())
            {
                _logger.LogWarning("Redaction request for document {DocumentId} has no valid phrases after filtering", id);

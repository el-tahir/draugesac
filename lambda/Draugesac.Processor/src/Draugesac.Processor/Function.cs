...
public class Function
{
    ...
    private async Task ProcessMessageAsync(SQSEvent.SQSMessage message, ILambdaContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Download original file from S3 with timeout
            ...
            string originalContent = await ReadStreamAsTextAsync(originalFileStream, cancellationToken).ConfigureAwait(false);
            ...
            using var redactedStream = new MemoryStream(Encoding.UTF8.GetBytes(redactedContent));

            _logger.LogInformation($"⬆️ Uploading redacted document to S3. Stream length: {redactedStream.Length}, Position: {redactedStream.Position}");
    ...
    private string PerformRedaction(string content, List<string> phrasesToRedact, CancellationToken cancellationToken = default)
    {
        ...
        for (int i = 0; i < phrasesToRedact.Count; i++)
        {
            ...
            string phrase = phrasesToRedact[i];
            if (string.IsNullOrWhiteSpace(phrase))
            {
                continue;
            }

            // Replace phrase with asterisks of same length
            string redaction = new string('*', phrase.Length);
            redactedContent = redactedContent.Replace(phrase, redaction, StringComparison.OrdinalIgnoreCase);

            // Log progress for long lists
            if (i % 10 == 0 && phrasesToRedact.Count > 20)
            {
